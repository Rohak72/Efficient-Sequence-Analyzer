...
def create_upload_file(file: UploadFile = File(..., description="User's FASTA Query"),
    ...
    try:
        s3_client.upload_fileobj(file.file, fasta_bucket_name, s3_key)
...
def read_file_contents(file_id: int, db: Session = Depends(get_db), 
    ...
    try:
        s3_object = s3_client.get_object(Bucket=fasta_bucket_name, Key=db_file.s3_key)
...
def read_download_file(file_id: int, db: Session = Depends(get_db), 
    ...
    try:
        ...
        url = s3_client.generate_presigned_url(
            'get_object',
            Params={'Bucket': fasta_bucket_name, 'Key': db_file.s3_key, 'ResponseContentDisposition': content_disposition},
...
def delete_file(file_id: int, db: Session = Depends(get_db), current_user: models.User = Depends(get_active_user)):
    ...
    try:
        s3_client.delete_object(Bucket=fasta_bucket_name, Key=db_file.s3_key)
    except ClientError as e:
